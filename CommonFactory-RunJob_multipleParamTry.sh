#!/bin/sh
# CommonFactory-RunJob.sh
# Author: Adil Baig (baigm01) 
# Creation Date: 2018/04/26
#
# Utility runs a ds job and returns the jobs status
#
# usage: sh ./CommonFactory-RunJob.sh <ProjectNameWithoutEnvpart> <commaSeperatedParameters> <JobName> <warningsOK[YES|NO]>
# e.g. :
# sh /opt/IBM/data/utils/CommonFactory-RunJob.sh "Dev_BWAdHoc" "SF" "ps_BWAdHoc=salesforce,jp_JOBNAME=Account,jp_DEBUG=1" "stg_SF_Lookup_Account_110_Extract" "YES"
# sh /opt/IBM/data/utils/CommonFactory-RunJob.sh "Dev_BWAdHoc" "SF" "ps_BWAdHoc=salesforce,jp_JOBNAME=Account,jp_DEBUG=1" "stg_SF_Lookup_Contact_110_Extract" "YES"
# sh /opt/IBM/data/utils/CommonFactory-RunJob.sh "BWAdHoc" "SF" "ps_BWAdHoc=salesforce,jp_JOBNAME=Account,jp_DEBUG=1" "stg_SF_Lookup_Account_110_warning" "NO"
# sh /opt/IBM/data/utils/CommonFactory-RunJob.sh "BWAdHoc" "SF" "ps_BWAdHoc=salesforce,jp_JOBNAME=Account,jp_DEBUG=1" "stg_SF_Lookup_Account_110_error" "NO"
# invalid param example, jpFILE does not apply to this job so that will be omitted:
# sh /opt/IBM/data/utils/CommonFactory-RunJob.sh "BWAdHoc" "SF" "ps_BWAdHoc=salesforce,jp_JOBNAME=Account,jp_DEBUG=1,jpFILE=file name.txt" "stg_SF_Lookup_Account_110_error" "NO"
# sh #ps_BWAdHoc.$env_BWADHOC_UTILS#/CommonFactory-RunJob.sh "#uvParams1.ProjectName#" "#uvParams1.FinalParamList#" "#slp_RunJobs.$Counter#" "#jp_WARNINGAOK#"
# sh /opt/IBM/data/utils//CommonFactory-RunJob.sh Dev_BWAdHoc SF ps_BWAdHoc=salesforce,jp_JOBNAME=Lookup,jp_DEBUG=0 Dev_BWAdHoc NO
...
printScriptHelpTxt()
{
	...
	if [ ${#varArgCount[@]} -lt 4 ]; then
		echo 'Aborting CommonFactory-RunJob.sh - invalid number of arguments'
		...
		echo 'e.g.: sh /opt/IBM/data/utils/CommonFactory-RunJob.sh "Prod_BWAdHoc" "ps_BWAdHoc=salesforce,jp_JOBNAME=Account,jp_DEBUG=1" "stg_SF_Lookup_Account_110_warning" "YES"'
...
if [[ "${varParameterValues}" =~ ";" ]]; then
	echo "passed parameter contains sub-parameters"
	read
fi
...
while IFS='' read -r eachParam || [[ -n "$eachParam" ]]; do
	...
	if [[ ${varParamName} == "ERROR" ]]; then
		rm -f ${varJobName}_${DATE_WITH_TIME}
		echo "Unable to read parameters for: $varJobName"
		 exit 2; 
	fi
	...
	do
		...
		#varPassedParamValuePart=${paramNameValue[1]}
		# if [[ "${paramNameValue[1]}" =~ "|" ]]; then
		# if [[ "${paramNameValue[1]}" == *[\;]* ]]; then
		char=";"
		#!/bin/bash
		if [[ "${paramNameValue[1]}" == *"${char}"* ]]; then
			echo "It contains semi colon"
			for i in $(echo ${paramNameValue[1]} | sed "s/;/ /g")
			do
...
while IFS='' read -r jobInfoLine || [[ -n "$jobInfoLine" ]]; do
	...
	if [[ ${jobInfoLine} == "Job Status"* ]] || [[ ${jobInfoLine} == "Status code"* ]]; then
		...
		if [ $varStatCode -eq 1002 ]; then
			echo "Invalid project: ${varProjectName}"
			rm -f ${varJobName}_${DATE_WITH_TIME}
			 exit 5
		fi
