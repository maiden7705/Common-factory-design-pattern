...
if (( $varStausCode != 1 )); then
	...
	[[ -z ${varOPTIONALbodyText} ]] && body="<html><body>The job <b>$varJobName</b> completed the run successfully !<br/></body></html>" || body="<html><body>${varOPTIONALbodyText}<br/><br/><strong>Job Run Status : <font size="3" color=\"green\">$varStatusText</font></strong><br/></body></html>"
fi

if (( $varStausCode == 100 )); then
	to=$varNotifEmails
	subject="${varCorrectedProjName} : ${varSubjectArea}_${varJobName} Run completed with rejects"
	[[ -z ${varOPTIONALbodyText} ]] && body="<html><body>The job <b>$varJobName</b> ran successfully with status : <b><font size="3" color=\"red\">COMPLETED WITH REJECTS</font></b><br/></body></html>" || body="<html><body>${varOPTIONALbodyText}<br/><br/><strong>Job Run Status : <font size="3" color=\"orange\">COMPLETED WITH REJECTS</font></strong><br/></body></html>"
fi

if (( $varStausCode == 150 )); then
	to=$varNotifEmails
	subject="${varCorrectedProjName} : ${varSubjectArea}_${varJobName} Run is in Progress"
	[[ -z ${varOPTIONALbodyText} ]] && body="<html><body>The job <b>$varJobName</b> Running with status : <b><font size="3" color=\"green\">IN PROGRESS</font></b><br/></body></html>" || body="<html><body>${varOPTIONALbodyText}<br/><br/><strong>Job Run Status : <font size="3" color=\"green\">IN PROGRESS</font></strong><br/></body></html>"
fi		

if [[ $varStausCode = "S" ]] || [[ $varStausCode = "C" ]]; then
	to=$varNotifEmails
	subject="${varCorrectedProjName} : ${varSubjectArea}_${varJobName} Run has $varStatusText"
	[[ -z ${varOPTIONALbodyText} ]] && body="<html><body>The job <b>$varJobName</b> has <strong>$varStatusText.</strong><br/><br/>" || body="<html><body>${varOPTIONALbodyText}<br/><br/><strong>Job Run Status : <font size="3" color=\"green\">$varStatusText</font></strong><br/></body></html>"
fi

if (( $varBadSubjctArea == 1 )); then
	to=$varFailureEmails
	subject="!! ${varCorrectedProjName} : ${varSubjectArea}_${varJobName} Load : $varJobName  - REPORTING EMAILS NOT FOUND !!"
	if [[ -z ${varOPTIONALbodyText} ]]; then 
		...
		For the supplied parameter value <strong>jp_REPORTING_SUBJECTAREA='$varSubjectArea'<strong><br/><br/><strong>Job Run Status : <font size="3" color=\"red\">FAILED</font></strong><br/></body></html>" 
		else
