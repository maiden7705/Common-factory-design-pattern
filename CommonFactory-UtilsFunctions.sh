...
getAllCountReportEmailBody()
{
	...
	local -r varEmailStyleTag="<style>thead {color:blank; align:left; background-color:LIGHTSKYBLUE} tbody {color:black; align:left} p.sideNote { font-style: italic } table, th, td { border: 2px solid black;align: left; } </style>"
	local -r varEmailTableHeaderTag="The <strong>${SubjectArea}_${JobNameMask}</strong> ran successfully, with following counts between good and rejected files:<br/><table><thead><tr><th><strong><span style='color: #993300;'>[STEPCODE]_[ACTION]</span></strong></th><th align='left'><span style='color:blank;'>TOTAL INPUT<br/>count to the step</span></th><th align='left'><span style='color:green;'>SUCCESS count</span></th><th align='left'><span style='color:red;'>REJECT count</span></th></tr></thead><tbody>"
	local -r varEmailBodyTemplate="<tr><td>STEPNAME</td><td>TOTALCOUNT</td><td>SUCCESSCOUNT</td><td>ERRORCOUNT</td></tr>"
	local -r varEmailClosingTag="</tbody></table><p class='sideNote'>NOTE: In the attached reject file:<br/>
	<strong>LEGENDS:</strong><br/>
	<strong><span style='color: #993300;'>[STEPCODE]</span></strong> : <strong>100*</strong> - Extraction, <strong>200*</strong> - Transformation, <strong>300*</strong> - Load<br/>
	<strong><span style='color: #993300;'>[ACTION]</span></strong> : Action being performed at that step<br/>
	...
	varEmailTemplateString="${varEmailStyleTag}${varEmailTableHeaderTag}"
	find ${RejectFolder} -name "sq_*${JobNameMask}_*.rej" -type f | sort -n | ( while read eachFile; do
		...
		# Trimming the 'sq_[jp_SUBJECTAREA]_*' part from the file name as that is redundant
		# to the job name in the email too
		ErrFileName="${ErrFileName%.*}"
		IFS='_' read -r -a fileNameParts <<< "$ErrFileName"
		varFILENAMETemplate=''
		varStartAppendingToName=0
		for fileNamePart in "${fileNameParts[@]}"
		do
			numericMatch='^[0-9]+$'
			if [ ${varStartAppendingToName} -eq 1 ]; then
				varFILENAMETemplate="${varFILENAMETemplate}_${fileNamePart}"
			fi
			
			if [[ $fileNamePart =~ $numericMatch ]] ; then
				varFILENAMETemplate="${fileNamePart}"
				varStartAppendingToName=1
			fi
		done
		#varFILENAMETemplate="${ErrFileName%.*}"
		errRowCount=`cat ${eachFile} | wc -l | tr -d '\n'`
